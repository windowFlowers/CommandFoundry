...
export function SettingsDrawer({ open, onClose, modelStatus, onChanged }) {
  ...
  return (
    <Drawer open={open} onClose={onClose} eyebrow="MODEL SETTINGS" title="DeepSeek 配置" label="设置" dataUi="settings-drawer">
      <div className="model-state compact-state"><span className={`status-dot ${modelStatus?.configured ? "ready" : ""}`} aria-hidden="true" /><strong>{modelStatus?.configured ? "deepseek-chat 已配置" : "当前使用本地回答"}</strong></div>
      ...
      <div className="drawer-actions">
        ...
        <button className="primary-button" type="button" disabled={working || !key.trim()} onClick={() => run("save")}>{working ? "处理中" : "保存"}</button>
      </div>
...
export function MemoryDrawer({ open, onClose, memory, loading, error, onReset }) {
  ...
  const statusText = memory?.status === "summarizing" ? "摘要中" : memory?.status === "pending" ? "等待摘要" : memory?.summary_provider === "deepseek" ? "DeepSeek 摘要" : memory?.summary_provider === "local" ? "本地摘要" : "最近对话";
